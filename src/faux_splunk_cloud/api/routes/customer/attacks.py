"""
Customer Attack Simulation API endpoints.

All operations are scoped to the customer's tenant.
Customers can only run attacks against their own instances.
"""
...
from faux_splunk_cloud.api.deps import get_actor_context
...
# =============================================================================
# Threat Actor Endpoints (Read-only, available to all customers)
# =============================================================================
...
async def create_campaign(
    ...
    The target instance must belong to the customer's tenant.
    """
    # Validate threat actor
    threat_actor = get_threat_actor_by_id(request.threat_actor_id)
    if not threat_actor:
        raise HTTPException(
            status_code=400,
    ...
    # Verify target instance belongs to customer's tenant
    await _verify_instance_ownership(request.target_instance_id, actor)
    ...
    # Create campaign config with tenant info
    ...
    try:
        ...
        # Store tenant_id with campaign for filtering
        campaign.tenant_id = actor.effective_tenant_id
    except ValueError as e:
...
async def list_campaigns(
    ...
    """
    List attack campaigns for the customer's tenant.

    Only campaigns targeting instances owned by this tenant are returned.
    """
    # Get campaigns, filtering by instance if provided
    campaigns = kill_chain_engine.list_campaigns(instance_id)

    # Filter to only campaigns targeting tenant's instances
    tenant_campaigns = []
    for campaign in campaigns:
        # Check if this campaign's target instance belongs to the tenant
        instance = await instance_manager.get_instance(campaign.target_instance_id)
        if instance:
            instance_tenant = instance.labels.get("tenant_id") if instance.labels else None
            if instance_tenant == actor.effective_tenant_id:
                tenant_campaigns.append(campaign)

    if status:
        tenant_campaigns = [c for c in tenant_campaigns if c.status == status]

    responses = [_campaign_to_response(c) for c in tenant_campaigns]
...
async def get_campaign(
    ...
    """Get a specific attack campaign (must target customer's instance)."""
    campaign = await _get_tenant_campaign(campaign_id, actor)
    return _campaign_to_response(campaign)
...
async def start_campaign(
    ...
    """Start a paused or pending campaign (must target customer's instance)."""
    await _get_tenant_campaign(campaign_id, actor)
...
async def pause_campaign(
    ...
    """Pause a running campaign (must target customer's instance)."""
    await _get_tenant_campaign(campaign_id, actor)
...
async def get_campaign_steps(
    campaign_id: str,
    actor: Annotated[ActorContext, Depends(get_actor_context)],
) -> list[AttackStepResponse]:
    """Get all steps in an attack campaign (must target customer's instance)."""
    campaign = await _get_tenant_campaign(campaign_id, actor)
...
async def get_campaign_logs(
    ...
    """Get logs generated by an attack campaign (must target customer's instance)."""
    await _get_tenant_campaign(campaign_id, actor)
...
async def execute_scenario(
    ...
    """
    # Verify target instance belongs to customer's tenant
    await _verify_instance_ownership(target_instance_id, actor)
...
async def _verify_instance_ownership(instance_id: str, actor: ActorContext) -> None:
    """Verify that the instance belongs to the customer's tenant."""
    ...
    if instance_tenant != actor.effective_tenant_id:
        raise HTTPException(
            status_code=403,
            detail="Cannot target instance belonging to another tenant",
        )


async def _get_tenant_campaign(campaign_id: str, actor: ActorContext) -> AttackCampaign:
    """Get campaign and verify it targets an instance in the customer's tenant."""
    campaign = kill_chain_engine.get_campaign(campaign_id)
    if not campaign:
        raise HTTPException(status_code=404, detail=f"Campaign {campaign_id} not found")

    # Verify target instance belongs to customer's tenant
    await _verify_instance_ownership(campaign.target_instance_id, actor)
